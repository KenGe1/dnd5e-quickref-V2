...
fetch(`https://pub-254bba07fa6840d4811b027ba5135175.r2.dev/json/${name}.json`)
  ...
  .then(data => {
    ...
    // 1. Alle Icon-Namen sammeln
    const iconList = data.map(item => item.icon);

    // 2. Duplikate entfernen
    const uniqueIcons = [...new Set(iconList)];

    // 3. Mapping für Anzeige-Namen (beliebig erweiterbar!)
    const iconLabels = {
      house: "Haus",
      bed: "Unterkunft",
      person: "Person",
    ...
    // Optional: alte Einträge leeren
    sideBar.innerHTML = "";

    // 4. Für jedes Icon HTML erzeugen
    uniqueIcons.forEach(iconName => {
      ...
      const p = document.createElement("p");
      p.textContent = iconLabels[iconName] || iconName;
